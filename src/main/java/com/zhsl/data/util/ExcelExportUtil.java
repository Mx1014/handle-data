...
public class ExcelExportUtil {
    ...
    public static void exportExcel(String title, Map<String, String> headMap, JSONArray jsonArray, String datePattern, int colWidth, OutputStream out) {
        ...
        workbook.getDocumentSummaryInformation().setCompany("智慧水利科技股份有限公司");
        SummaryInformation si = workbook.getSummaryInformation();
        si.setAuthor("xianjg");  //填加xls文件作者信息
     ...
     * @param out         输出流
     */
    public static void exportExcelX(String title, List<HeaderVO> headMap, JSONArray jsonArray, String datePattern, int colWidth, OutputStream out) {
        if (datePattern == null) datePattern = DEFAULT_DATE_PATTERN;
        // 声明一个工作薄
...
//        cellStyle.setFillPattern(FillPatternType.SOLID_FOREGROUND);
        ...
        String[] headers = new String[headMap.size()];
        int ii = 0;
        for (HeaderVO headerVO:headMap) {
            properties[ii] = headerVO.getFiled();
            headers[ii] = headerVO.getText();
            int bytes = headerVO.getFiled().getBytes().length;
            arrColWidth[ii] = bytes < minBytes ? minBytes : bytes;
            sheet.setColumnWidth(ii, arrColWidth[ii] * 256);
            ii++;
        }
    ...
    //Web 导出excel
    public static void downloadExcelFile(String title, List<HeaderVO> headMap, JSONArray ja, String path) {
        try {
